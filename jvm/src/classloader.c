				...
				char *filed_name)
{
	...
	/*
	 * list_head中保存的是全局变量jvm_class_list_head的引用，
	 * list_head是CLASS的list,这里遍历每个CLASS，在遍历CLASS中的CLASS_FILED
	 * 获取class_filed中的name_base和传入的filed_name进行比较,如果比对成功这返回CLASS_FILED的引用
	 *
	 */
	list_for_each(q, list_head) {
		r = list_entry(q, CLASS, list);
		/*({(CLASS *)((char * )q - (char *)&(((CLASS *)0x0)->list)); });*/
				...
				char *method_name)
{
	...
	/*
	 * list_head中保存的是全局变量jvm_class_list_head的引用，
	 * list_head是CLASS的list,这里遍历每个CLASS，在遍历CLASS中的CLASS_METHOD
	 * 获取CLASS_METHOD中的name_base和传入的method_name进行比较,如果比对成功这返回CLASS_METHOD的引用
	 *
	 */
	list_for_each(q, list_head) {
		r = list_entry(q, CLASS, list);
...
CLASS *jvm_parse_class_file(const char *class_file, const char *class_name)
{
	...
	//读取class文件到内存，内存开始地址的指针为class_start_mem
	if (mmap_class_file(class_file) == -1)
			return NULL;
	//分配一个java.class文件的描述结构
	 ...
	 *	CLASS类中的list代表了 一个list结构，
	 * 	将新分配的new_class内存空间加入到全局的链表(jvm_class_list_head)中去,形成一个双向链表关系
	 **/

	list_add_tail(&(new_class->list), &jvm_class_list_head);
	//拷贝class的路径名称到class的内存中
	strcpy(new_class->class_file, class_name);
		//class_start_mem是读取class文件到内存是生成的内存的基地址
        ...
        if (parse_class_version(new_class) == -1)
                goto out;
        //常量信息，首先获取常量的个数信息，在从第一个常量开始读取，因为知道每个常量的长度和
        //和第一个常量的开始地址，故可以逐个解析常量信息到class分配的内存中
